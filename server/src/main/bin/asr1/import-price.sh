...
sqlplus -S vivo_bck/password@DEVEL01 @conf/sql/truncate-service-price.sql

echo Clean Table: prc_service
sqlplus -S vivo_bck/password@DEVEL01 @conf/sql/delete-service.sql

echo Clean Table: prc_service_free_units
sqlplus -S vivo_bck/password@DEVEL01 @conf/sql/delete-service-free-units.sql
...
echo renaming first file so it will not be handled in the for bellow
mv xaa firstfile
cd -
...
ImportId=`sqlplus -S vivo_bck/password@DEVEL01 @conf/sql/select-import-id.sql | tail -2 | head -1 | sed -e 's/^ *//' -e 's/ *$//'`
